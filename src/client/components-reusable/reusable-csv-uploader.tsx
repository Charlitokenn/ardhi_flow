...
import {addSpacesBeforeCapitals, cn} from "@/lib/utils";
// ============================================================================
...
function StepIndicator({
    ...
    if (state === "active") {
        return (
            <div
                className="relative flex size-8 items-center justify-center shrink-0"
                aria-current="step"
            >
                {/* Static track */}
                ...
                {/* Rotating ring */}
                <CircleDashed
                    className="absolute inset-0 size-8 animate-spin animation-duration-[3s]"
                    strokeWidth={2}
                />
            ...
            {isHorizontal ? (
                            ...
                            return (
                                <div key={step.title} className="flex items-center gap-3">
                                    ...
                                    {index < steps.length - 1 && (
                                        <div
                                            className={cn(
                                                "-mt-3.5 h-0.5 w-10 transition-colors",
                ...
                steps.map((step, index) => {
                    ...
                    return (
                        <div
                            key={step.title}
                            className="flex items-start gap-3 relative flex-1"
                        >
                            {/* Indicator */}
...
export interface CsvUploaderProps<TInsert extends Record<string, unknown>> {
    /** Name of the entity being uploaded, e.g. "Contact". Only used for copy. */
    entityName: string;
     ...
     * May return a per-row created/skipped/failed summary, which the uploader
     * renders instead of its generic "imported" toast; returning nothing
     * keeps the previous generic toast behavior.
     */
    onSubmit: (
        rows: TInsert[],
    ) => Promise<CsvImportSummary | void> | CsvImportSummary | void;
}
...
export function ReusableCSVUploader<TInsert extends Record<string, unknown>>({
    ...
    const [mapping, setMapping] = useState<
        Record<string, string | "__ignore" | null>
    >({});
    ...
    const handleFile = useCallback(
        async (file: File) => {
            ...
            const scoreMatch = (header: string, candidate: string) => {
                ...
                if (cTokens.length && cTokens.every((t) => hTokens.includes(t)))
                    return 50;
                return 0;
            };
                ...
                const scored = parsed.headers
                    .map((h) => {
                        const bestScore = Math.max(
                            ...candidates.map((c) => scoreMatch(h, c)),
                        );
                        return {header: h, score: bestScore};
                    })
                    .filter((x) => x.score > 0)
                    .sort(
                        (a, b) => b.score - a.score || a.header.length - b.header.length,
                    );
    ...
    const handleConfirmAndSubmit = useCallback(async () => {
        ...
        try {
            const summary = await onSubmit(resultRows);

            if (summary) {
                // A summary was returned — show the created/skipped/failed
                // breakdown instead of the generic toast (see
                // docs/specs/0001-contacts-completion/0005-contacts-csv-bulk-import.md).
                setImportSummary(summary);
                setIsSubmitting(false);
                return;
            }

            toast(`${entityName} imported`, {
    ...
    if (importSummary) {
        const skippedResults = importSummary.results.filter(
            (r) => r.status === "skipped",
        );
        const failedResults = importSummary.results.filter(
            (r) => r.status === "failed",
        );
        const problemRows = importSummary.results.filter(
            (r) => r.status !== "created",
        );
        ...
        return (
            <div className="m-4 space-y-4">
                ...
                <div className="flex flex-wrap gap-2">
                    ...
                    <Badge variant="destructive">Failed: {failedResults.length}</Badge>
                </div>

                {problemRows.length > 0 && (
                    <ScrollArea className="h-[360px] pe-3.5">
                        <Table>
                            <TableHeader>
                                <TableRow>
                            ...
                            <TableCaption>
                                Rows not listed here were created successfully.
                            </TableCaption>
                        </Table>
                    </ScrollArea>
                )}
    ...
    return (
        <div
            ref={layoutRef}
            className={cn("flex gap-6 m-4", isWideLayout ? "flex-row" : "flex-col")}
        >
            {/* Stepper */}
            <div
                className={cn("mt-2", isWideLayout ? "min-w-[220px] w-auto" : "w-full")}
            >
                <CsvStepper
                    currentStep={step}
                    steps={steps}
            ...
            <div className="flex-1 space-y-6 mt-2">
                ...
                {step === 1 && (
                    <section aria-labelledby={dropzoneLabelId} className="space-y-4">
                        ...
                        <div className="flex items-center gap-2 text-xs text-muted-foreground">
                            ...
                            <span>
                Ensure your first row contains column headers. You can map them
                in the next step.
              </span>
                        </div>
                    </section>
                                ...
                                <TableBody>
                                    {fields.map((field) => {
                                        ...
                                        const isRequiredMissing =
                                            field.required &&
                                            (!mappedHeader || mappedHeader === "__ignore");

                                        return (
                                            <TableRow
                                                key={field.key}
                                                className={isRequiredMissing ? "bg-destructive/5" : ""}
                                            >
                                                <TableCell>
              ...
              <span className="text-xs text-muted-foreground">
                {requiredFieldsMissing.length > 0 && (
                    ...
                    <span className="font-medium text-destructive">
                      {requiredFieldsMissing.length} required field
                        {requiredFieldsMissing.length > 1 ? "s" : ""} missing
                      mapping.
                    </span>{" "}
                    ...
                    <section className="space-y-4">
                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between text-xs">
                                ...
                                <span className="text-muted-foreground">
                  Showing {preview.length} of {preview.length} row
                                    {preview.length === 1 ? "" : "s"}
                </span>
                            </div>
                            ...
                            {isSubmitting && (
                                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                    <CircleDashed className="size-4 animate-spin animation-duration-[3s]"/>
                                    <span>
                    Uploading {csvRows.length} row
                                        {csvRows.length === 1 ? "" : "s"}...
                  </span>
                                </div>
                            )}
                ...
                {error && (
                    <div
                        className="mt-2 flex items-start gap-2 rounded-md border border-destructive/40 bg-destructive/5 px-3 py-2 text-xs text-destructive">
