...
export function ProjectsDataGrid() {
  ...
  const data = useMemo<IProject[]>(() => (projectsQuery.data as IProject[]) ?? [], [projectsQuery.data])
  ...
  const columns = useMemo<ColumnDef<DataGridFeatures, IProject>[]>(
    () => [
          ...
          (row) => (
            <div className="flex items-center gap-3">
              <Avatar className="size-8">
                <AvatarFallback>{initials(row.original.projectName)}</AvatarFallback>
              </Avatar>
              <div className="space-y-px">
                <div className="text-foreground font-medium">{row.original.projectName}</div>
      ...
      },
      {
        ...
        id: "numberOfPlots",
        header: ({ column }) => (
          <DataGridColumnHeader title="Plots" visibility={true} column={column} />
        ),
        cell: renderWithDeleteSkeleton(
          <Skeleton className="h-7 w-auto" />,
          (row) => (
            <div className="text-foreground font-medium">
              {row.original.numberOfPlots}
              {row.original.plots?.length ? (
                <span className="text-muted-foreground"> ({row.original.plots.length} recorded)</span>
              ) : null}
            </div>
          )
        ),
      ...
      },
      {
        ...
        id: "acquisitionValue",
        header: ({ column }) => (
          <DataGridColumnHeader title="Acquisition Value" visibility={true} column={column} />
        ),
        cell: renderWithDeleteSkeleton(
          <Skeleton className="h-7 w-auto" />,
          (row) => (
        ...
        cell: renderWithDeleteSkeleton(
          ...
          (row) => (
            <div className="flex items-center gap-1.5">
              {statusBadge(row.original.tpStatus)}
              {statusBadge(row.original.surveyStatus)}
            </div>
          )
        ),
        size: 200,
      ...
      },
      {
        ...
        id: "acquisitionDate",
        header: ({ column }) => (
          <DataGridColumnHeader title="Acquired" visibility={true} column={column} />
        ),
        cell: renderWithDeleteSkeleton(
          <Skeleton className="h-7 w-auto" />,
          (row) => (
  ...
  const handleBulkDelete = () => {
    ...
    Promise.all(selectedIds.map((id) => deleteProject.mutateAsync(id))).then(() => {
      queryClient.invalidateQueries({ queryKey: ["projects"] })
      setDeletingIds((prev) => {
        const next = new Set(prev)
  ...
  return (
    <>
      ...
      >
        <TableActionBar table={table} onExport={() => exportSelected("projects")} onDelete={handleBulkDelete} />
        ...
        formContent={
          viewingRow && (
            <div className="space-y-2 text-sm">
              <div><span className="text-muted-foreground">Project Name: </span>{viewingRow.projectName}</div>
              <div><span className="text-muted-foreground">Details: </span>{viewingRow.projectDetails ?? "—"}</div>
              <div><span className="text-muted-foreground">Owner: </span>{viewingRow.projectOwner ?? "—"}</div>
              ...
              <div><span className="text-muted-foreground">Ward: </span>{viewingRow.ward ?? "—"}</div>
              <div><span className="text-muted-foreground">Number of Plots: </span>{viewingRow.numberOfPlots}</div>
              <div><span className="text-muted-foreground">Sqm Bought: </span>{viewingRow.sqmBought ?? "—"}</div>
              <div><span className="text-muted-foreground">Acquisition Value: </span>{formatCurrency(viewingRow.acquisitionValue)}</div>
              <div><span className="text-muted-foreground">Acquisition Date: </span>{formatDate(viewingRow.acquisitionDate)}</div>
              <div><span className="text-muted-foreground">TP Status: </span>{viewingRow.tpStatus ?? "—"}</div>
              <div><span className="text-muted-foreground">Survey Status: </span>{viewingRow.surveyStatus ?? "—"}</div>
            </div>
          )
        }
      />
      ...
      >
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete {deletingRow?.projectName}? This action cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
