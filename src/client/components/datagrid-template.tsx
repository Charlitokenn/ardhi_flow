  ...
  return (
      <DropdownMenu>
        ...
        <DropdownMenuContent side="left" align="start">
          {/* EDIT: opens the sheet for THIS row */}
...
export function DataTablePattern() {
  ...
  const filteredData = useMemo(() => {
    return demoData.filter((item) => {
      ...
      // Use optional chaining in case joinedDate is missing
      const matchesDate = matchesDateRange(item.joinedDate, joinedRange)

      return matchesStatus && matchesSearch && matchesDate
    })
  }, [searchQuery, selectedStatuses, joinedRange])
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
            <ActionsCell
                row={row}
                onEdit={(data) => setEditingRowRef.current(data)}
            />
        ),
        size: 60,
        enableSorting: false,
